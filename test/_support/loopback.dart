...
/// the server root (always has a trailing slash) so callers can do
/// `serverUri.resolve('-/npm/v1/keys')` without thinking about port
/// formatting.
///
/// Tests are responsible for `await server.close(force: true)` (in a
/// `tearDown` / `finally` block) — leaks here will exhaust the
/// macOS ephemeral port range across CI runs.
